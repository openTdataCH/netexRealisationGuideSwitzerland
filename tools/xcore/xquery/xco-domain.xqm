...
(:~
 ...
 :
 : @param reportType a report type
 : @param delimiter a file name delimiter, e.g. '.'
...
declare function dm:getReportPartPath($reportType as xs:string,
   ...
   let $mainReportPath := dm:getReportPath($reportType, $domain, $options)
    return
        ...
        else fu:insertLabelBeforeFileNameExtension($mainReportPath, $delimiter||$reportPart)
};
...
declare function dm:getReportPartPath($reportType as xs:string,
                                      ...
                                      $options as map(xs:string, item()*))
        as xs:string {
            dm:getReportPartPath($reportType, '.', $reportPart, $domain, $options)
        };
