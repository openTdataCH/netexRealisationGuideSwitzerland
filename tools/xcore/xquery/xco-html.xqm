    ...
    let $xsdDivs :=
        ...
        let $tables :=
            ...
            let $compNr := 
                ...
                return $cnr + $shift
            let $table := hl:contabReport_complexComp($comp, $compNr, $schemaNr, $domain, $options)

            (: NeTEx: create individual file for each complex type table :)
            let $html := if ($options?htmlFilePerComplexType) then (
                let $compName := hl:strip_prefix($comp/@z:name,':')
                let $_LOG := trace($compName)
                ...
                return <a href="{$compHref}"><h3>{$compKind || " " || $compName}</h3></a>
            )
            else (
                $table
            )
...
declare function hl:strip_prefix($s as xs:string?, $sep as xs:string) as xs:string? {
  if (empty($s)) then ()
  else replace($s, '^[^'|| $sep ||']*'||$sep, "")
};
            ...
            return
                <li>
                    ...
                    <ul class="sectlevel2">{
                        ...
                        let $idPrefix := $ttp[3]
                        (: NeTEx: create individual file for each complex type table :)
                        let $compHref := if ($options?htmlFilePerComplexType) then (
                            (: TODO refactor to function compHref:)
                            let $compName := fn:tokenize($comp/@z:name/fn:string(),':')[1]
                            let $domainName := $domain/processing/reportFileBaseName
                            return $domainName || '/' || $compKind ||'s/'|| $compName || '.html'
                        ) else (
                           '#'||$idPrefix||$name
                        )
                        return
                        <li>
                            <a shape="rect" 
                               href="{$compHref}">{$snr}.{$compNr} {$txt1} <code>{$dispName}</code>{$txt2}
                            </a>
                        </li>
                    }</ul>
               </li>
