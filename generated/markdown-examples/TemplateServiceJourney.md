...
| ++ | [AvailabilityCondition](AvailabilityCondition.md) | expected | 1..1 | None | VALIDITY CONDITION stated in terms of DAY TYPES and PROPERTIES OF DAYs. | **TODO** Expand the element, see ServiceJourney: more spacific requirements than standard AvailabilityCondition. |
...
| ++ | [NoticeAssignment](NoticeAssignment.md) | optional | 1..1 | None | The assignment of a NOTICE showing an exception in a JOURNEY PATTERN, a COMMON SECTION, or a VEHICLE JOURNEY, possibly specifying at which POINT IN JOURNEY PATTERN the validity of the NOTICE starts and ends respectively. |  |
...
| ++ | [TimetabledPassingTime](TimetabledPassingTime.md) | expected | 1..1 | None | TIMETABLED PASSING TIME at TIMING POINT. |  |
...
| ++ | RhythmicalJourneyGroup | expected | 1..1 | None | A group of VEHICLE JOURNEYS following the same JOURNEY PATTERN having the same "rhythm" every hour (for example runs all xxh10, xxh25 and xxh45... e) between a specified start and end time. |  |
...
| ++ | HeadwayJourneyGroup | optional | 1..1 | None | A group of VEHICLE JOURNEYs following the same JOURNEY PATTERN and having the same headway interval between a specified start and end time (for example, ‘every 10 minutes’). This is especially useful for presenting passenger information. |  |
