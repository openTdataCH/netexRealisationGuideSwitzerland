...
| + | PrivateCode | expected | 1..1 | PrivateCodeStructure | A private code that uniquely identifies the element. May be used for inter-operating with other (legacy) systems. | when the id is not a sloid, then if a sloid is relevant it can be added here |
|  | Name | optional | 0..1 | MultilingualString | Name of Traveller | The names are the same in all languages. Can be omitted as this is taken from the StopPlace/Quay |
