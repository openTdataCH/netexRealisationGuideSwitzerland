...
* delivery as network, operator or lines.
...
| IT-Environment                 | DEV,TEST,INT,PROD |                                                                 |
|--------------------------------|-------------------|-----------------------------------------------------------------|
...
# Zip structure
We propose different structures for different scenarios:
...
Beside the sender of the message (system), participants must also identify the environment from which the message is sent. Both parts are concatenated by a “_”.
Example:
...
Other platform identifiers can be used only after mutual agreement. It is not necessary for a partner to have all these environments. However, a mapping between the two involved environment sets is necessary.
...
-	Export from SKI will also be provided via https://opentransportdata.swiss/ for download.

> **TODO** we will change this to S3.
