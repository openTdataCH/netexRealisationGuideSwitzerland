...
# -*- coding: utf-8 -*-
"""
Generate a profile XSD 1.0 from a base XSD and a Schematron overlay.
...
Usage:
    python schema-generator.py --base StopPlace.xsd --sch Restrictions.sch --out StopPlaceRestricted.xsd
"""
...
def resolve_qname_in_rule(token: str, rule: ET.Element) -> ET.QName | None:
...
def find_global_element(xsd_root: ET.Element, lname: str) -> ET.Element | None:
    """Find top-level xs:element by local name."""
    elems = xsd_root.findall(f"./xs:element[@name='{lname}']", namespaces=NS)
    return elems[0] if elems else None


def element_complex_type(xsd_root: ET.Element, elem: ET.Element) -> ET.Element | None:
    ...
    if t:
        ...
        types = xsd_root.findall(f"./xs:complexType[@name='{type_local}']", namespaces=NS)
...
def find_child_decl(ctype: ET.Element, lname: str) -> ET.Element | None:
    """Find a child xs:element with given name inside the complex type's xs:sequence (first match)."""
    if ctype is None:
        return None
    elems = ctype.findall(f".//xs:sequence//xs:element[@name='{lname}']", namespaces=NS)
    return elems[0] if elems else None


def context_complex_type(xsd_root: ET.Element, context_locals: list[str]) -> ET.Element | None:
...
def classify_assert(assert_el: ET.Element) -> dict | None:
    """
...
def get_diagnostic_snippet(sch_tree: ET.ElementTree, assert_el: ET.Element) -> ET.Element | None:
    ...
    if not ids:
        return None
    for d in ids:
        diags = sch_tree.xpath(f"//sch:diagnostic[@id='{d}']", namespaces=NS)
        for diag in diags:
...
def ensure_min_occurs_one(elem: ET.Element) -> bool:
...
def remove_child_elements(ctype: ET.Element, child_local: str) -> int:
    """Remove all occurrences of xs:element[@name=child_local] under ctype/xs:sequence. Returns count removed."""
    count = 0
    nodes = ctype.findall(f".//xs:sequence//xs:element[@name='{child_local}']", namespaces=NS)
    for n in nodes:
        parent = n.get("..")
...
def insert_child_element(ctype: ET.Element, new_elem: ET.Element) -> bool:
...
def default_new_element(child_local: str) -> ET.Element:
...
def apply_operation(xsd_root: ET.Element,
                    sch_tree: ET.ElementTree,
...
def collect_ops_from_schematron(sch_tree: ET.ElementTree) -> list[tuple[list[str], dict]]:
    """
    Iterate Schematron rules and collect applicable operations.
    Returns list of tuples: (context_locals, op_dict).
    """
    ops = []
    rules = sch_tree.findall("//sch:rule", namespaces=NS)
    for rule in rules:
        context = rule.get("context") or ""
        context_locals = parse_context_to_locals(context)
        for assert_el in rule.findall("./sch:assert", namespaces=NS):
...
def main():
    ...
    if not ops:
        ...
        xsd_tree.write(args.out, encoding="UTF-8", xml_declaration=True)
        return
